...
规范结果为 `{ todos, counts: { pending, inProgress, completed } }`，其中 `counts` 汇总任意深度的每个节点；其 Native 渲染器返回精简的更新确认。工具还会写入完整 `todo/tree` 会话事件。目前没有任何已交付的渲染器消费该事件：想要持久树形视图的 host 需自行订阅事件流并渲染。浏览器客户端每次调用的 `TodoRow` 是唯一确实反映树形的已交付界面，因为它以本包与扁平工具共享的工具 **名称** 为键，且读取的是调用参数而非事件；它用显式栈统计 `children`（它读取的参数未经校验，递归遍历会把渲染调用栈撑爆），因此那一行摘要汇总了每个节点。
...
- **扁平 UI 部署看不到内容**：UI 只有在实现树渲染器时才会渲染 `todo/tree` 事件；扁平 `todo/write` 渲染器按设计忽略此事件。
